...
const Login = () => {
	...
	const spectatorHr = async () => {
		setSpinner1(true);
		const data = await loginFetcher.spectatorHr();
		await direct(data);
	};

	const spectatorAc = async () => {
		setSpinner2(true);
		const data = await loginFetcher.spectatorAc();
		await direct(data);
	};
							...
							<div class="form-container">
								<Form>
									<Form.Group controlId="formBasicEmail">
										...
										/>
										<Form.Text className="text-muted">enter your mail.</Form.Text>
									</Form.Group>
									...
									<button onClick={() => login()} type="button" class="btn" id="close">
										{spinner ? (
											<Spinner
												as="span"
												animation="border"
